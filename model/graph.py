...
def build_deepwalk_corpus_random_for_large_bibartite_graph(G, hits_dict, percentage, maxT, minT, alpha=0, rand = random.Random(), node_type='u'):
  walks = []
  nodes_total = list(G.nodes()) # G.nodes is empty
  nodes = []
  # print(len(nodes),node_type)
  for obj in nodes_total:
    # todo 6. nodes_total has no u and i as a first string
    #       eg. u1 = user-1 ,   i4 = item-3
    #       > Add u to gene and i to disease. # apply dict {key: i#}
    if obj[0] == node_type:
      nodes.append(obj)
  # cnt_0 = 1
  # print(len(nodes))
